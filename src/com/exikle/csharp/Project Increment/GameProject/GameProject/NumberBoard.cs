...
namespace GameProject {
    ...
    class NumberBoard {
        ...
        public NumberBoard(ContentManager contentManager, Vector2 center, int sideLength,
            ...
            drawRectangle.Width = (int) center.X * 2;
            drawRectangle.Height = (int) center.Y * 2;

            tileSideLength = (sideLength - ((NUM_COLUMNS + 1) * BORDER_SIZE)) / 3;

            // calculate side length for number tiles

            for (int x = 0; x < NUM_ROWS; x++) {
                for (int y = 0; y < NUM_COLUMNS; y++) {
                    Vector2 tileCenter = CalculateTileCenter(x + 1, y + 1);
                    tiles[x, y] = new NumberTile(contentManager, tileCenter, tileSideLength, 8, 8, null);
                }
            }
        ...
        public void Draw(SpriteBatch spriteBatch) {
            ...
            // draw the board,spriteBatch.Begin is already called
