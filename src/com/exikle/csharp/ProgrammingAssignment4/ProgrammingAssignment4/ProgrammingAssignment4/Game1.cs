...
namespace ProgrammingAssignment4 {
    ...
    public class Game1 : Microsoft.Xna.Framework.Game {
        ...
        protected override void Update(GameTime gameTime) {
            ...
            if (gameState == GameState.Play) {
                if (whoseTurn == PlayerType.Human) {
                    ...
                    bool turnTaken = board.TakePlayerTurn(mouse);

                    //IF THE PLAYER TOOK A TURN, CHANGES WHOSE TURN IT IS
                    if (turnTaken) {
                    ...
                    //THE COMPUTER PLAYER TAKES A TURN AND CHANGE WHOSE TURN IT IS
                    computer.TakeTurn(board);
                    ChangeTurn();
                }
                if (board.GameOver()) {

                    //sets the quite button to be visible and changes the state to the game over state
                    gameState = GameState.GameOver;
                    qButton.Visible = true;
                }

            } else if (gameState == GameState.GameOver) {
                qButton.Update(mouse);
            } else {
        ...
        protected override void Draw(GameTime gameTime) {
            ...
            bSquare.Draw(spriteBatch);

            qButton.Visible = true;
            if (gameState == GameState.GameOver) {
                qButton.Draw(spriteBatch);
            }
