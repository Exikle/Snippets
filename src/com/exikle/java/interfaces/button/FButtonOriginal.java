...
public class FButtonOriginal extends JButton implements Runnable {
	...
	GradientPaint gp2 = new GradientPaint(0, 0, Color.LIGHT_GRAY, 80,
			85, Color.GRAY, false);
	...
	public void paintComponent(Graphics g) {
		...
		RadialGradientPaint rgp1 = new RadialGradientPaint(new Point(
				getWidth() / 2, getHeight() / 2), radius,
				new float[] { 0.1f, .55f }, new Color[] {
						new Color(0, 0, 0, .1f), ICE_BLUE });
		RadialGradientPaint rgp2 = new RadialGradientPaint(new Point(
				getWidth() / 2, getHeight() / 2), radius,
				new float[] { 0.1f, .55f }, new Color[] {
						new Color(0, 0, 0, .1f), Color.GRAY });
		...
		g2.fill(new Arc2D.Float(x, y, radius - 10, radius - 10, 0,
				360, Arc2D.PIE));
		...
		g2.drawString(label, (getWidth() / 2) - 17,
				(getHeight() / 2) + 2);
	}
